...
def double_threshold(improved,class_name,filename,num):
    ...
    for i in range(0, nf):
        ...
        if ste[i] > ste_low and stz[i] > stz_low:  # 判断低阈值
            ...
            # print(ste[i],stz[i])
            segment_end = i
            if segment_end - segment_start > 10 and segment_end - segment_start < 500:  # 判断持续时间
                if improved == 1:
                    if len(segments) >= 1:
    ...
    title_font = font_manager.FontProperties(family='serif', style='normal', size=30)
    ticks_font = font_manager.FontProperties(family='serif', style='normal', size=30)
    # 获取声音信号的时间轴
    ...
    for segment in segments:
        plt.axvline(x=segment[0] * (inc * 1.0 / framerate) / 2, color="g", linestyle="--",linewidth=3)
        plt.axvline(x=segment[1] * (inc * 1.0 / framerate) / 2, color="g", linestyle="--",linewidth=3)
        # print(segment[0],segment[1])
    plt.grid(False)
    plt.title("Sound waveform",fontproperties=title_font)
    if improved == 0:
        plt.savefig(f"sample\双阈值三阈值法对比结果\双阈值\{class_name}_double.jpg",dpi = 1200)
    else:
        plt.savefig(f"sample\双阈值三阈值法对比结果\三阈值\{class_name}_triple.jpg",dpi = 1200)
...
for i in range(1):
    class_name = "crow"
    filename = f"sample/双阈值三阈值法对比结果/crow.wav"   #拿这个样本来得到截取区间
    double_threshold(0,class_name=class_name,filename=filename,num=i+1)